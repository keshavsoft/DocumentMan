...
let WithOutScreen = async ({ inJsonConfig, inItemConfig, inUserPK, inPostData }) => {
    ...
    if (inUserPK > 0) {
        ...
        //LocalUpdatedData[inItemConfig.inItemName] = { ...LocalUpdatedData[inItemConfig.inItemName], ...inPostData };
       // LocalUpdatedData[inItemConfig.inItemName] = { ...inPostData, ...LocalUpdatedData[inItemConfig.inItemName] };

        for (const property in LocalUpdatedData[inItemConfig.inItemName]) {
            for (const property1 in inPostData) {
                LocalUpdatedData[inItemConfig.inItemName][property1]=inPostData[property1];
